...
fn parse_opf(
    xml: &str,
) -> Result<EpubPackage, Error> {
    let document = Document::parse(xml)
        .map_err(|error| {
            Error::Parser(format!(
                "failed to parse EPUB OPF: {}",
                error,
            ))
        })?;
    ...
    package.title =
        find_metadata_text(&document, "title");

    package.author =
        find_metadata_text(&document, "creator");

    package.description =
        find_metadata_text(&document, "description");

    package.language =
        find_metadata_text(&document, "language");
...
fn find_cover_id(
    ...
    // Последний fallback — изображение,
    // в имени которого встречается "cover".
    manifest
        .iter()
        .find(|(_, item)| {
            let href = item.href
...
fn find_opf_path(
    ...
    let document = Document::parse(container_xml)
        .map_err(|error| {
            Error::Parser(format!(
                "failed to parse EPUB container.xml: {}",
                error,
            ))
        })?;
    ...
    document
        ...
        .ok_or_else(|| {
            Error::InvalidDocument(
                "EPUB OPF path not found in container.xml"
                    .to_owned(),
            )
        })
}
