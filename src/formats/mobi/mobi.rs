...
impl Loader for MobiLoader {
    ...
    fn load(
        ...
        let language = normalize_mobi_language(
            &book,
        );
...
fn split_html_into_chapters(
    ...
    for (index, pagebreak) in document
        .select(&pagebreak_selector)
        .enumerate()
    {
        let Some(span) =
            pagebreak
