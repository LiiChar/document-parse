...
impl Loader for DocxLoader {
    ...
    fn load(
        ...
        let metadata =
            extract_metadata(&mut archive, path)?;
    ...
    let Some(target) =
        docx.document_rels
            ...
            .and_then(|relationships| {
                relationships.get_target(embed)
            })
    else {
        return Ok(None);
    };
...
fn get_list_kind(
    ...
    let Some(level) =
        numbering.level.as_ref()
    else {
        return "bullet".to_owned();
    };

    let Some(numbering) =
        docx.numbering.as_ref()
    else {
        return "bullet".to_owned();
    };

    let Some(details) =
        numbering.numbering_details(
            num_id.value,
        )
    else {
        return "bullet".to_owned();
    ...
    let format = details
        ...
        .iter()
        .find(|level_details| {
            level_details.i_level
                == Some(level.value)
        })
        .and_then(|level_details| {
            level_details.number_format.as_ref()
        })
        .map(|format| {
            format.value.as_ref()
        });
