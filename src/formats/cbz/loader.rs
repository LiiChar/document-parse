    ...
    fn load(&self, path: &Path, _options: &ParseOptions) -> Result<RawDocument, Error> {
        ...
        for index in 0..archive.len() {
            let mut entry = archive.by_index(index).map_err(|error| {
                Error::Parser(format!("failed to read CBZ entry {}: {}", index, error,))
            })?;
            ...
            entry.read_to_end(&mut data).map_err(|error| {
                Error::Parser(format!("failed to read CBZ image '{}': {}", name, error,))
            })?;
            ...
            pages.push(Page { name, data });
        }

        pages.sort_by(|left, right| compare(&left.name, &right.name));

        if pages.is_empty() {
            return Err(Error::InvalidDocument(
                "CBZ archive does not contain images".to_owned(),
            ));
        }
        ...
        for (index, page) in pages.iter().enumerate() {
            let resource_id = format!("page-{}", index + 1);

            resources.push(RawResource {
                id: resource_id.clone(),
                mime_type: guess_mime(&page.name).to_owned(),
