...
fn extract_metadata(
    ...
    RawMetadata {
        ...
        description: extract_annotation(
            document,
        ),
...
fn extract_author(
    ...
    let first_name = child_text(
        author,
        "first-name",
    );

    let last_name = child_text(
        author,
        "last-name",
    );

    let nickname = child_text(
        author,
        "nickname",
    );
...
fn read_fb2_from_zip(
    ...
    Err(Error::InvalidDocument(
        "FB2 document was not found inside ZIP archive"
            .to_owned(),
    ))
}
